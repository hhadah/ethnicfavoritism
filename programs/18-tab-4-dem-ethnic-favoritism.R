...
reg1 <- list(
  "\\specialcell{(1) \\\\ Schooling}"             = feols(CompletedPrimaryEdu ~ coethnic*Polity + urban + female | birth_year + age + EthClusters, data = DHS_PrimSchl, vcov = ~EthClusters),
  "\\specialcell{(2) \\\\ Infant Survival}"       = feols(infant_survival     ~ coethnic*Polity + kidbord + urban + kidsex | birth_year + age + EthClusters, data = DHS_Infantdata_polity_coethnic, vcov = ~EthClusters),
  "\\specialcell{(3) \\\\ Wealth}"                = feols(Richest ~ coethnic*Polity + urban + female | birth_year + age + EthClusters, data = DHS_Wealth, vcov = ~EthClusters),
  "\\specialcell{(4) \\\\ Electrification}"       = feols(Electrification ~ coethnic*Polity + urban + female | birth_year + age + EthClusters, data = DHS_ElectrificationWater, vcov = ~EthClusters),
  "\\specialcell{(5) \\\\ Clean Water}"           = feols(AccessToWater ~ coethnic*Polity + urban + female | birth_year + age + EthClusters, data = DHS_ElectrificationWater, vcov = ~EthClusters)
...
attr(mean_row, 'position') <- c(4)
cm <- c("coethnic:Polity" = "$PolityIV\\times Coethnic$"
...
modelsummary(reg1, fmt = 2, 
             ...
             title = "Ethnic favoritism and continuous democracy measure results. \\label{tab:ethdemcont}") %>%
...
regression_tab %>%
  save_kable(file.path(tables_wd,"tab04-ethnic_demfav_cont.tex"))

regression_tab %>%
  save_kable(file.path(thesis_tabs,"tab04-ethnic_demfav_cont.tex"))
