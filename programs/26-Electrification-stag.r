...
# feols(Electrification ~ coethnic + urban + female | birth_year + age + EthClusters, data = DHS_ElectrificationWater, vcov = ~EthClusters)
...
mod4 =
  etwfe(
    fml  = Electrification ~ 1, #urban + female + age, # outcome ~ controls
...
ggplot(mod4_es |> filter(event <= 20), aes(x = event, y = estimate, ymin = conf.low, ymax = conf.high)) +
  ...
  theme(axis.title = element_text(color = "black",  size = 9)) +
  theme(plot.title = ggtext::element_markdown(size = 9, color = "black", hjust = 0, lineheight = 1.2)) +
  ggtitle(paste("Extended Two Way Fixed Effects DiD on electriication"))
ggsave(paste0(figures_wd,"/elec-DiD-etwfe-treat-es.png"), width = 12, height = 4, units = "in")
ggsave(paste0(thesis_plots,"/elec-DiD-etwfe-treat-es.png"), width = 12, height = 4, units = "in")
